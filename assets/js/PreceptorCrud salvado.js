...
window.addNuevopreceptor = async function (event) {
    ...
    // LEER DIRECTAMENTE DEL INPUT (Sin usar fd.get)
    const nombreParaEnviar = document.getElementById("preceptor").value;
    const turnoParaEnviar = document.getElementById("turno").value;
    const obsParaEnviar = document.getElementById("obs").value;

    console.log("Enviando estos textos:", nombreParaEnviar, turnoParaEnviar);

    try {
        // Llamada limpia
        await addNuevopreceptor(nombreParaEnviar, turnoParaEnviar, obsParaEnviar);
        
        // Cerrar modal y limpiar
        bootstrap.Modal.getInstance(document.getElementById("agregarPreceptorModal")).hide();
        document.getElementById("formularioPreceptor").reset();
...
// assets/js/PreceptorCrud.js

// Buscamos el formulario por su ID
const formulario = document.getElementById("formularioPreceptor");

if (formulario) {
    formulario.addEventListener("submit", async (e) => {
        e.preventDefault(); // Detiene la recarga de página y el envío nativo

        const fd = new FormData(formulario);
        
        // Obtenemos los strings limpios de los campos 'name'
        const nombre = fd.get("preceptor");
        const turno = fd.get("turno");
        const obs = fd.get("obs");

        // VALIDACIÓN DE SEGURIDAD: Si no hay datos, no disparamos Firebase
        if (!nombre || !turno) {
            console.error("No se capturaron los datos. Revisa los atributos 'name'");
            return;
        }

        try {
            // Llamamos a la función de firebase.js enviando SOLO los strings
            await addNuevopreceptor(nombre, turno, obs);

            // Cerramos modal y reseteamos
            const modalEl = document.getElementById("agregarPreceptorModal");
            bootstrap.Modal.getInstance(modalEl).hide();
            formulario.reset();

            await refrescarTablas();
            window.mostrarAlerta({ tipoToast: "success", mensaje: "¡Guardado!" });
        } catch (error) {
