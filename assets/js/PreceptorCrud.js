...
import {
  addPreceptor,
  getOtrosPreceptoresCollection,
  deletePreceptorCollection,
  updatePreceptorCollection,
  getPreceptorCollection,
} from "./firebase.js";
...
window.miModal = async function (idModal, idRef = "") {
  try {
    ...
    if (idModal === "detallePreceptorModal")
      await cargarDetallePreceptor(idRef);
    if (idModal === "editarPreceptorModal")
      await getPreceptorUpdateCollection(idRef);
    ...
    console.error("Error Modal Preceptor:", error);
  }
};
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      const contenedor = document.querySelector(
        "#detallePreceptorContenido ul",
      );
      if (!contenedor) return;
      contenedor.innerHTML = ` 
                <li class="list-group-item"><b>Apellido y Nombre: </b> ${data.nombre || "N/A"}</li>
                <li class="list-group-item"><b>Turno:</b> ${data.turno || "Mañana"}</li>
                <li class="list-group-item"><b>Cargo:</b> ${data.cargo || "Preceptor"}</li>
                ...
                <li class="list-group-item"><b>Obs </b> ${data.obs || "Ninguna"}</li>`;
    }
  } catch (error) {
    console.error("Error detalles preceptor:", error);
  }
}
      ...
      if (formEdit) {
        ...
        if (selectTurno) {
          selectTurno.value = data.turno;
           ...
           const selectCargo = formEdit.querySelector("#cargo");
        if (selectCargo) {
          selectCargo.value = data.cargo;
        }
      }
    }
  } catch (error) {
...
async function eliminarPreceptor(id) {
  try {
    ...
    console.error("Error al borrar preceptor:", error);
  }
}
