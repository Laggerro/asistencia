  ...
  try {
    ...
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
        ...
        fila.id = id;
        fila.innerHTML = `
                    <td>${curso.curso || "No definido"}</td>
                    <td>${curso.ubicacion || "No definido"}</td>
                    <td>${curso.capacidad || "No definida"}</td>
                    <td>${curso.obs || "Ninguna"}</td>
                    <td>
...
window.miModal = async function (idModal, idRef = "") {
  try {
    ...
    if (idModal.includes("eliminar")) {
      ...
      if (btn)
        btn.onclick = async () => {
          idModal.includes("alumno")
            ? await eliminaralumno(idRef)
            : await eliminarCurso(idRef);
          myModal.hide();
        };
    }
  } catch (error) {
...
window.addNuevoCurso = async function (event) {
  ...
  try {
    // Captura el valor y limpia espacios en blanco
    const obsValue = fd.get("obsCurso")?.trim();

    // Si está vacío (string vacío ""), asigna "Ninguna"
    const observacionFinal = obsValue ? obsValue : "Ninguna";
    ...
    bootstrap.Modal.getInstance(
      document.getElementById("agregarCursoModal"),
    ).hide();
...
window.addNuevoalumno = async function (event) {
  ...
  try {
    ...
    bootstrap.Modal.getInstance(
      document.getElementById("agregaralumnoModal"),
    ).hide();
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    ...
    ?.addEventListener("input", mostraralumnosEnHTML);
  document
    .getElementById("filtroCurso")
    ?.addEventListener("change", mostraralumnosEnHTML);
...
async function getCursoUpdateCollection(id) {
  try {
    ...
    if (cursoDoc.exists()) {
      ...
      if (document.querySelector("#horaIn"))
        document.querySelector("#horaIn").value = data.horaIn || "";
...
window.actualizaralumno = async function (event) {
  ...
  try {
    ...
    bootstrap.Modal.getInstance(
      document.getElementById("editaralumnoModal"),
    ).hide();
    window.mostrarAlerta({
      tipoToast: "success",
...
window.actualizarCurso = async function (event) {
  ...
  try {
    ...
    bootstrap.Modal.getInstance(
      document.getElementById("editarCursoModal"),
    ).hide();
    window.mostrarAlerta({
      tipoToast: "success",
